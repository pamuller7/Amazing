    ...
    def update_dir(self, way_to_open: int) -> None:
        ...
        if way_to_open == self.maze.north:
            self.maze.put_in_maze(
                [self.pos_line - 1, self.pos_col], self.maze.south
            )
            self.pos_line -= 1
        elif way_to_open == self.maze.south:
            self.maze.put_in_maze(
                [self.pos_line + 1, self.pos_col], self.maze.north
            )
            self.pos_line += 1
        elif way_to_open == self.maze.west:
            self.maze.put_in_maze(
                [self.pos_line, self.pos_col - 1], self.maze.east
            )
            self.pos_col -= 1
        elif way_to_open == self.maze.east:
            self.maze.put_in_maze(
                [self.pos_line, self.pos_col + 1], self.maze.west
            )
            self.pos_col += 1
